...
class Test_WPML_Elementor_Media_Node_Image extends OTGS_TestCase {
	...
	public function it_should_not_translate_if_properties_missing_or_empty() {
		...
		$this->assertEquals( $expected_settings, $subject->translate( $settings, 'fr', 'en' ) );
	}
	...
	private function get_media_translate( $ids_map = null, $urls_map = null ) {
		...
		if ( $ids_map ) {
			$media_translate->method( 'translate_id' )->willReturnMap( $ids_map );
		}

		if ( $urls_map ) {
			$media_translate->method( 'translate_image_url' )->willReturnMap( $urls_map );
		}

		return $media_translate;
	}
}
